...
5. **基于深度学习的语言模型**:在最近的发展中，一种基于深度学习的自然语言处理技术被用于表示文字，包括汉字。这种技术将每个字或词映射到一个高维向量(称为词嵌入)，这个向量可以捕捉到字或词的语义信息。
...
- 我们会把数据(文本,图片..)存成向量的方式给到大模型, 那大模型如何才能知道这些词的意思和关系呢,就需要表示学习理解方式(如“国王 - 男人 + 女人 ≈ 女王”）实际的数据维度是非常高的 
...
- 在创建的应用当中选择知识库,选择模型,在点击发布

![](images/033.png)

- 使用千问中的自己创建的应用，可以在本地调用API 
...
response = Application.call(
    ...
    app_id='5889632592634a2d9a79344161588639',  # 替换为实际的应用 ID
    prompt='公司的请假流程？')
