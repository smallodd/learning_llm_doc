...
**Tavily** 是一款专为 AI 代理（Agent）和 RAG 系统设计的**实时搜索引擎 API**。

与传统搜索 API 不同，Tavily 能自动完成搜索、抓取、内容提取和筛选，直接向大语言模型（LLM）返回最相关、结构化的信息，而非原始链接。这能有效减少 AI 的“幻觉”并确保信息时效性。

它提供 Search、Extract、Crawl 三大核心 API，与 LangChain、LlamaIndex 等主流框架深度集成，具备 SOC 2 认证。Tavily 已服务于 Cohere、IBM 等企业，并提供每月 1000 次搜索的免费套餐。他的访问地址为 https://app.tavily.com/home

**代码示例**
```python
# tavily是一个搜索工具可以从网上在线搜索文档，需要注册使用  
import os  
from dotenv import load_dotenv  
...
search_str = TavilySearch(tavily_api_key=os.getenv("TAVILY_API_KEY"))  
res = search_str.invoke("如何使用python")  
print(res)
```
###### 2.3.2 旅行助手案例 
