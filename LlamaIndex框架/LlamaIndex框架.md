...
| Document | Rank in A（Vector） | Rank in B（BM25） |
| -------- | ----------------- | --------------- |
| doc1     | 1                 | 2               |
| doc2     | 2                 | 1               |
| doc3     | 3                 | 4               |
...
| Document | RRF Score (approx)    |
| -------- | --------------------- |
| doc1     | 1/61 + 1/62 ≈ 0.03252 |
| doc2     | 1/62 + 1/61 ≈ 0.03252 |
| doc5     | 1/65 + 1/63 ≈ 0.03080 |
| doc3     | 1/63 + 1/64 ≈ 0.03100 |
| doc4     | 1/64 + 1/65 ≈ 0.03056 |
...
| 模式名称                      | 核心逻辑                              | 优点                        | 缺点                   | 适用场景                    |
| ------------------------- | --------------------------------- | ------------------------- | -------------------- | ----------------------- |
| REFINE (迭代优化)             | 逐节点迭代优化：先用第一个节点生成初始答案，再结合后续节点逐次优化 | 答案精度高、信息利用充分              | LLM 调用次数多（N-1 次）、速度慢 | 对答案精度要求高，节点数量少的场景       |
| COMPACT (紧凑优化)            | 先合并文本块到上下文窗口上限，再迭代优化              | 速度比 REFINE 快，精度仍较高        | 合并过程略复杂              | 中等数量节点，追求精度与速度平衡        |
| SIMPLE_SUMMARIZE (简单汇总)   | 合并所有文本块为一个，单次 LLM 调用生成答案          | 速度最快、调用次数最少               | 易超出上下文窗口上限导致失败       | 文本块总长度≤LLM 上下文窗口，追求极致速度 |
| TREE_SUMMARIZE (树状汇总)     | 构建树索引（自底向上），以查询为种子生成汇总，返回根节点结果    | 适配大量文本块，信息整合性强            | 逻辑复杂、耗时较长            | 海量文本块，需深度整合多节点信息        |
| GENERATION (纯生成)          | 忽略上下文，仅用 LLM 生成答案                 | 无需处理上下文、响应极快              | 答案与上下文无关，精度无保障       | 仅需基于问题本身生成通用回答          |
| NO_TEXT (仅返回节点)           | 不合成答案，仅返回检索到的上下文节点                | 无 LLM 调用成本、保留原始信息         | 无最终答案，需用户自行处理        | 需人工审核 / 二次处理检索结果的场景     |
| CONTEXT_ONLY (仅返回文本)      | 拼接所有文本块为字符串返回，不生成答案               | 保留完整原始文本、无合成偏差            | 无结构化答案，需用户自行总结       | 需完整获取原始上下文的场景           |
| ACCUMULATE (累加合成)         | 为每个文本块单独生成答案，最终拼接所有答案             | 保留每个节点的独立答案、信息无丢失         | LLM 调用次数多（N 次）、速度慢   | 需保留各节点独立结论，不追求统一答案      |
| COMPACT_ACCUMULATE (紧凑累加) | 先合并文本块到上下文窗口上限，再为每个合并块生成答案，最终拼接   | 速度比 ACCUMULATE 快，保留块级独立答案 | 合并过程略复杂，答案仍为拼接形式     | 中等数量节点，需块级独立答案且追求速度     |
...
| 评估阶段 | 指标名称              | 定义/公式                      | 意义             |
| ---- | ----------------- | -------------------------- | -------------- |
| 检索   | Precision（精确度）    | 检索到的相关文档数 / 检索到的文档总数       | 衡量检索结果的“纯度”    |
| 检索   | Recall（召回率）       | 检索到的相关文档数 / 数据库中相关文档总数     | 衡量检索结果的“查全率”   |
| 响应   | Faithfulness（忠实度） | 答案中能从上下文中推导出的事实数 / 答案中事实总数 | 衡量模型是否存在“胡说八道” |
| 响应   | Relevance（相关性）    | 基于语义嵌入的余弦相似度               | 衡量答案是否切中用户痛点   |
